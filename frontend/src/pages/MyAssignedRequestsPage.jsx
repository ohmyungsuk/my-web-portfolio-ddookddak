...
const FILTER_OPTIONS = [
  { key: "all", label: "전체" },
  { key: "assigned_group", label: "배정/협의" },
  { key: "planned", label: "작업 예정" },
  { key: "in_progress", label: "작업 진행중" },
...
function getProgressInfo(status) {
  ...
  if (["assigned", "quoted", "planned"].includes(normalized)) {
    return {
      percent: 50,
      activeStep: 1,
      color: BRAND,
...
function getWorkMessage(status) {
  const normalized = normalizeStatus(status);

  if (normalized === "assigned") {
    return "배정된 요청이에요. 상세보기에서 작업 예정으로 변경할 수 있어요.";
  }

  if (normalized === "quoted") {
    return "요청을 확인했어요. 작업 전 내용을 조율하는 단계예요.";
  }

  if (normalized === "planned") {
    return "작업 예정 상태예요. 상세보기에서 작업을 시작할 수 있어요.";
  }

  if (normalized === "in_progress") {
    return "작업이 진행 중이에요. 완료되면 완료 처리할 수 있어요.";
  }
  ...
  return (
    ...
    >
      ...
      <div style={styles.infoLine}>
        ...
        <span>{getWorkMessage(request.status)}</span>
      </div>
...
export default function MyAssignedRequestsPage({ onGoHome, onClickRequest }) {
  ...
  }, [location.state, fetchAssignedRequests]);
  ...
  const summary = useMemo(() => {
    ...
    return {
      ...
      assigned: normalized.filter((status) =>
        ["assigned", "quoted", "planned"].includes(status),
      ).length,
      working: normalized.filter((status) => status === "in_progress").length,
    ...
    return requests.filter((item) => {
      ...
      const statusMatch =
        ...
        (selectedFilter === "assigned_group" &&
          ["assigned", "quoted"].includes(normalized));
    ...
    <div style={styles.page}>
      <div style={styles.container}>
        <div style={styles.topArea}>
          <div>
            <h1 style={styles.title}>내가 맡은 작업</h1>
            <div style={styles.subTitle}>
              내가 수락한 유지보수 요청을 카드로 빠르게 확인해요.
            </div>
          </div>
        ...
        <div style={styles.summaryBar}>
          <div style={styles.summaryItem}>
            <div style={styles.summaryLabel}>전체 작업</div>
            <div style={styles.summaryValue}>{summary.total}개</div>
          </div>

          <div style={styles.summaryItem}>
            <div style={styles.summaryLabel}>배정/예정</div>
            <div style={styles.summaryValue}>{summary.assigned}개</div>
          </div>

          <div style={styles.summaryItem}>
            <div style={styles.summaryLabel}>작업 진행</div>
            <div style={styles.summaryValue}>{summary.working}개</div>
          </div>
        ...
        <p style={styles.countText}>
          총 {filteredRequests.length}개의 작업이 보여요.
        </p>
